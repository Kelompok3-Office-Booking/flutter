import 'package:betterspace/src/utils/hex_color_convert.dart';
import 'package:flutter/material.dart';

class MyColor {
  static HexColor redColor = HexColor('FF5C3C');
  static HexColor darkColor = HexColor('121721');
  static HexColor dark700Color = HexColor('3A3E46');
  static HexColor grayLightColor = HexColor('6E7A95');
  static HexColor whiteColor = HexColor('FCFCFC');
  static HexColor darkBlueColor = HexColor('2943A3');
  static HexColor netralColor = HexColor('F5F5F5');
  static HexColor secondary500ColorWaitStatusBorder = HexColor('4D62B2');
  static HexColor secondary900ColorWaitStatusBody = HexColor('D4D9ED');
  static HexColor success300ColorSuccessStatusBorder = HexColor('39923D');
  static HexColor success600ColorSuccesStatusBody = HexColor('A2D7A4');
  static HexColor danger200ColorFailStatusBorder = HexColor("9B4232");
  static HexColor danger600ColorFailStatusBody = HexColor("F4B1A5");
  static Color transparanColor = Colors.transparent;
}
